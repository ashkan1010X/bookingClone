...
export default function AccountNav() {
  ...
  function headingSelector(type = null) {
    let classStyle =
      "inline-flex gap-2 py-3 px-6 rounded-full transition-all duration-200 ease-in-out ";
    if (type === subpage) {
      classStyle +=
        " bg-gradient-to-r from-purple-500 to-purple-900 text-white shadow-lg";
    } else {
      classStyle +=
      ...
      <nav className="flex gap-8 mb-8 bg-gradient-to-r from-purple-700 to-purple-900 p-4 rounded-full shadow-lg">
        <Link className={headingSelector("profile")} to={"/account"}>
          <svg
            ...
            fill={subpage === "profile" ? "white" : "none"}
            viewBox="0 0 24 24"
            strokeWidth={1.5}
            stroke="currentColor"
            className="w-6 h-6"
